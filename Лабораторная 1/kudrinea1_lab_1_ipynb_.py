# -*- coding: utf-8 -*-
"""KudrinEA1_lab_1.ipynb"

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Ax9IOj1vu2UReOVvMjHpGOJ9KG96bk0e
...
def task_1(cities):
  capitalize_cities = [cities.capitalize() for cities in list_cities]
...
def task_2(name, population):
    biger_million_cities = [cities for cities in name if cities["population"] >= population]
    return biger_million_cities
...
def task_4(cities, find_c):
    lower_city_name = [cities.lower() for cities in cities]
    city_counter = lower_city_name.count(find_c.lower())
    return city_counter
...
def green_index(cities):
    for city in cities:                                 # Суммируем значение зеленых зон
        city['green_zones'] = sum(city['green_zones'])

    for city in cities:                                 # Вычисляем Зеленый индекс для каждого города
        city['green_index'] = city['territory_area'] / city['green_zones']

    for city in cities:                               # Оставляем название города и зел.индекс
        del city['territory_area']
        del city['green_zones']
    return cities


def mid_green_index(cities):
    green_indexes = [city['green_index'] for city in cities]  # Выделяем зел.индексы в отдельный список
    mid_green_index = round(sum(green_indexes) / len(green_indexes), 4) # Вычисляем среднеий зел.индекс
    return mid_green_index


first_step = green_index(list_territories)
second_step = mid_green_index(first_step)
